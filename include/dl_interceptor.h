...
// Returns 0 on success, -1 on failure.
DI_EXPORT int dl_interceptor_register_dl_init_callback(dl_interceptor_callback_t pre,
...
DI_EXPORT int dl_interceptor_unregister_dl_init_callback(dl_interceptor_callback_t pre,
                                               dl_interceptor_callback_t post, void *data);
...
//   data - opaque pointer passed back to callbacks
// Returns 0 on success, -1 on failure.
DI_EXPORT int dl_interceptor_register_dl_fini_callback(dl_interceptor_callback_t pre,
                                             dl_interceptor_callback_t post, void *data);

// Unregister a previously registered dl_fini callback.
// Returns 0 on success, -1 if not found.
DI_EXPORT int dl_interceptor_unregister_dl_fini_callback(dl_interceptor_callback_t pre,
                                               dl_interceptor_callback_t post, void *data);
...
//   other - execute the returned function pointer instead (can be original or replacement)
...
// The hook is called for every .init_array function of every library loaded after registration.
// The hook can inspect, replace, or skip each init function.
// Returns 0 on success, -1 on failure.
DI_EXPORT int dl_interceptor_register_init_array_hook(dl_interceptor_array_hook_t hook, void *data);

// Unregister a previously registered init_array hook.
// Returns 0 on success, -1 if not found.
DI_EXPORT int dl_interceptor_unregister_init_array_hook(dl_interceptor_array_hook_t hook, void *data);
...
// When the matching library is loaded and the specified init_array index is about
// to execute, the process will SIGSTOP itself, allowing a debugger to attach.
//
