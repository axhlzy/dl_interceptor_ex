...
// execution
//
// Implementation: hooks soinfo::call_constructors() and
// soinfo::call_destructors() in the Android linker via DobbyHook. Uses xDL for
// symbol resolution and runtime memory scanning to discover soinfo struct field
// offsets (vendor-independent).
...
static int di_cb_list_remove(di_callback_list *list,
                             dl_interceptor_callback_t pre,
                             dl_interceptor_callback_t post, void *data) {
  pthread_rwlock_wrlock(&list->lock);
...
static void di_cb_list_invoke_pre(di_callback_list *list,
                                  struct dl_phdr_info *info, size_t size) {
  pthread_rwlock_rdlock(&list->lock);
  for (di_callback *cur = list->head; cur; cur = cur->next) {
    if (cur->pre)
      cur->pre(info, size, cur->data);
  }
  pthread_rwlock_unlock(&list->lock);
}

static void di_cb_list_invoke_post(di_callback_list *list,
                                   struct dl_phdr_info *info, size_t size) {
  pthread_rwlock_rdlock(&list->lock);
  for (di_callback *cur = list->head; cur; cur = cur->next) {
    if (cur->post)
      cur->post(info, size, cur->data);
  }
  pthread_rwlock_unlock(&list->lock);
}
...
static int di_array_hook_list_add(di_array_hook_list *list,
                                  dl_interceptor_array_hook_t hook,
                                  void *data) {
...
static int di_array_hook_list_remove(di_array_hook_list *list,
                                     dl_interceptor_array_hook_t hook,
                                     void *data) {
  pthread_rwlock_wrlock(&list->lock);
...
static bool di_soinfo_is_loading(void *soinfo) {
  auto val = *reinterpret_cast<uint8_t *>(reinterpret_cast<uintptr_t>(soinfo) +
                                          g_off_constructors_called);
  return val == 0;
}
...
static void di_soinfo_to_dlinfo(void *soinfo, struct dl_phdr_info *info) {
  ...
  info->dlpi_phnum =
      static_cast<ElfW(Half)>(*reinterpret_cast<size_t *>(base + g_off_phnum));
}
...
static int di_soinfo_scan_pre(void *soinfo) {
  ...
  for (size_t i = 0; i < dlinfo.dlpi_phnum; i++) {
    if (dlinfo.dlpi_phdr[i].p_type == PT_DYNAMIC) {
      l_ld = reinterpret_cast<uintptr_t>(dlinfo.dli_fbase) +
             dlinfo.dlpi_phdr[i].p_vaddr;
      break;
    }
  }
  if (l_ld == UINTPTR_MAX) {
  ...
  for (size_t i = 0; i < sizeof(uintptr_t) * DI_SOINFO_SCAN_WORDS;
       i += sizeof(uintptr_t)) {
    ...
    uintptr_t v2 =
        *reinterpret_cast<uintptr_t *>(si + i + sizeof(uintptr_t) * 2);
    uintptr_t v5 =
        *reinterpret_cast<uintptr_t *>(si + i + sizeof(uintptr_t) * 5);
    uintptr_t v6 =
        *reinterpret_cast<uintptr_t *>(si + i + sizeof(uintptr_t) * 6);
    ...
    // Pattern 2: link_map layout: load_bias, l_name, l_ld, l_next, l_prev(0),
    // load_bias_again Matches: load_bias == dli_fbase, l_ld == PT_DYNAMIC addr,
    // l_prev == 0, load_bias field == load_bias
    if (g_off_load_bias == SIZE_MAX && v0 == known_load_bias && v2 == l_ld &&
        v5 == 0 && v6 == known_load_bias) {
      ...
      if (name_len >= dummy_len &&
        ...
        // In AOSP: link_map { l_addr, l_name, l_ld, l_next, l_prev } then
        // constructors_called
  ...
  if (g_off_load_bias == SIZE_MAX || g_off_name == SIZE_MAX ||
      g_off_phdr == SIZE_MAX || g_off_phnum == SIZE_MAX ||
      g_off_constructors_called == SIZE_MAX) {
    DI_LOGE("scan: failed to discover offsets (load_bias=%zu, name=%zu, "
            "phdr=%zu, phnum=%zu, called=%zu)",
            g_off_load_bias, g_off_name, g_off_phdr, g_off_phnum,
            g_off_constructors_called);
    return -1;
  }

  DI_LOGI("scan: offsets discovered (load_bias=%zu, name=%zu, phdr=%zu, "
          "phnum=%zu, called=%zu)",
          g_off_load_bias, g_off_name, g_off_phdr, g_off_phnum,
          g_off_constructors_called);
  return 0;
...
static void di_soinfo_scan_post(void *soinfo) {
  auto val = *reinterpret_cast<uintptr_t *>(
      reinterpret_cast<uintptr_t>(soinfo) + g_off_constructors_called);
  if (val != 0) {
...
static void di_log_init_details(const struct dl_phdr_info *info) {
  ...
  for (const ElfW(Dyn) *d = dyn; d->d_tag != DT_NULL; d++) {
    ...
    case DT_INIT_ARRAYSZ:
      ...
      break;
    case DT_FINI:
      fini_func = d->d_un.d_ptr;
      break;
    case DT_FINI_ARRAY:
      fini_array = d->d_un.d_ptr;
      break;
    case DT_FINI_ARRAYSZ:
      fini_arraysz = d->d_un.d_val;
      break;
    }
  }
  ...
  if (init_array && init_arraysz) {
    ...
    DI_LOGI("  .init_array    : %zu entries at rel=0x%" PRIxPTR, count,
    ...
    for (size_t i = 0; i < count; i++) {
      ...
      if (sym)
        DI_LOGI("    [%zu] abs=0x%" PRIxPTR " rel=0x%" PRIxPTR " %s", i,
                (uintptr_t)func, (uintptr_t)rel, sym);
      else
        DI_LOGI("    [%zu] abs=0x%" PRIxPTR " rel=0x%" PRIxPTR, i,
                (uintptr_t)func, (uintptr_t)rel);
    }
  }
  ...
  if (fini_array && fini_arraysz) {
    ...
    DI_LOGI("  .fini_array    : %zu entries at rel=0x%" PRIxPTR, count,
...
static ElfW(Addr) * di_apply_init_array_hooks(const struct dl_phdr_info *info) {
  ...
  // Make the init_array page writable (it's in a RELRO segment on newer
  // Android)
  uintptr_t page_size = (uintptr_t)sysconf(_SC_PAGESIZE);
  uintptr_t page_start = (uintptr_t)arr & ~(page_size - 1);
  size_t region_size = ((uintptr_t)arr + count * sizeof(ElfW(Addr)) -
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    dl_interceptor_init_func_t replacement =
        reinterpret_cast<dl_interceptor_init_func_t>(orig_func);
...
static void di_restore_init_array(const struct dl_phdr_info *info,
                                  ElfW(Addr) * backup) {
  if (!backup)
    return;
  ...
  if (scan_tid == 0) {
    ...
    if (g_offsets_ready.load(std::memory_order_relaxed)) {
      if (di_soinfo_is_loading(soinfo)) {
        ...
        DI_LOGI("ctors: pre  [%s] load_bias=0x%" PRIxPTR, info.dlpi_name,
...
static void proxy_call_destructors(void *soinfo) {
  if (g_offsets_ready.load(std::memory_order_relaxed) &&
    ...
    auto val = *reinterpret_cast<uint8_t *>(
        reinterpret_cast<uintptr_t>(soinfo) + g_off_constructors_called);
    if (val != 0) {
      struct dl_phdr_info info = {};
      di_soinfo_to_dlinfo(soinfo, &info);

      // Ignore: call_destructors() during failed load (constructors_called ==
      // 0, name/addr == 0)
...
static int di_hook_linker() {
  ...
  const char *ctors_sym =
      (api >= __ANDROID_API_M__) ? DI_SYM_CALL_CTORS_M : DI_SYM_CALL_CTORS_L;
  void *ctors_addr = xdl_dsym(linker, ctors_sym, nullptr);

  // Resolve soinfo::call_destructors
  const char *dtors_sym =
      (api >= __ANDROID_API_M__) ? DI_SYM_CALL_DTORS_M : DI_SYM_CALL_DTORS_L;
  ...
  // Hook with mini_hook
  if (mini_hook_install(
          ctors_addr, reinterpret_cast<void *>(proxy_call_constructors),
  ...
  if (mini_hook_install(
          dtors_addr, reinterpret_cast<void *>(proxy_call_destructors),
...
static int di_discover_offsets() {
  ...
  // Load the dummy library — this triggers call_constructors, which runs our
  // scan
...
int dl_interceptor_register_init_array_hook(dl_interceptor_array_hook_t hook,
                                            void *data) {
  if (!hook)
    return -1;
...
int dl_interceptor_unregister_init_array_hook(dl_interceptor_array_hook_t hook,
                                              void *data) {
...
static void di_dump_library(const di_wrapper_ctx *ctx) {
  ...
  DI_LOGI("  dump: wrote %s (%zu bytes, base=0x%" PRIxPTR ")", dump_path,
          dump_size, (uintptr_t)ctx->lib_base);
}
...
static void di_init_entry_wrapper(void) {
  ...
  if (ctx->is_dumppoint) {
    DI_LOGI("  >>> DUMP [%zu/%zu] abs=0x%" PRIxPTR " rel=0x%" PRIxPTR, disp_idx,
            disp_total, (uintptr_t)ctx->func_abs, (uintptr_t)ctx->func_rel);
    di_dump_library(ctx);
  }

  if (ctx->is_breakpoint) {
    DI_LOGI("  >>> BREAKPOINT [%zu/%zu] abs=0x%" PRIxPTR " rel=0x%" PRIxPTR,
            disp_idx, disp_total, (uintptr_t)ctx->func_abs,
            (uintptr_t)ctx->func_rel);
    DI_LOGI("  >>> Process stopped (pid=%d). Resume: kill -CONT %d", getpid(),
            getpid());
    raise(SIGSTOP);
    DI_LOGI("  >>> Resumed");
  }
  ...
  if (ctx->orig_func) {
    ...
    long long elapsed_ns =
        (long long)(end_ts.tv_sec - start_ts.tv_sec) * 1000000000LL +
        (end_ts.tv_nsec - start_ts.tv_nsec);
    double elapsed_ms = (double)elapsed_ns / 1000000.0;
    DI_LOGI("  done init_array[%zu/%zu] took %.3f ms", disp_idx, disp_total,
            elapsed_ms);
  } else {
...
static bool di_has_breakpoint_for_lib(const char *lib_name) {
  ...
  for (int i = 0; i < DI_MAX_BREAKPOINTS; i++) {
    if (g_breakpoints[i].active &&
        strstr(lib_name, g_breakpoints[i].lib_name) != nullptr) {
      pthread_mutex_unlock(&g_bp_lock);
      return true;
    }
    if (g_dumppoints[i].active &&
        strstr(lib_name, g_dumppoints[i].lib_name) != nullptr) {
      pthread_mutex_unlock(&g_bp_lock);
      return true;
    }
...
static bool di_breakpoint_matches(const char *lib_name, size_t index) {
  ...
  for (int i = 0; i < DI_MAX_BREAKPOINTS; i++) {
    ...
    if (strstr(lib_name, g_breakpoints[i].lib_name) != nullptr) {
      if (g_breakpoints[i].index == -1 ||
          g_breakpoints[i].index == (int)index) {
        pthread_mutex_unlock(&g_bp_lock);
        return true;
      }
    }
...
di_breakpoint_hook(const struct dl_interceptor_array_entry *entry, void *data) {
  ...
  if (entry->index < DI_MAX_INIT_ENTRIES) {
    ...
    ctx->orig_func =
        reinterpret_cast<dl_interceptor_init_func_t>(entry->func_abs);
...
int dl_interceptor_set_breakpoint(const char *lib_name, int index) {
  ...
  strncpy(g_breakpoints[slot].lib_name, lib_name,
          sizeof(g_breakpoints[slot].lib_name) - 1);
...
int dl_interceptor_set_dumppoint(const char *lib_name, int index) {
  ...
  strncpy(g_dumppoints[slot].lib_name, lib_name,
          sizeof(g_dumppoints[slot].lib_name) - 1);
...
int dl_interceptor_set_hookpoint(const char *lib_name) {
  ...
  if (slot < 0) {
    pthread_mutex_unlock(&g_bp_lock);
    DI_LOGE("hookpoint: no free slots (max=%d)", DI_MAX_BREAKPOINTS);
    return -1;
  }
  strncpy(g_hookpoints[slot].lib_name, lib_name,
          sizeof(g_hookpoints[slot].lib_name) - 1);
  g_hookpoints[slot].lib_name[sizeof(g_hookpoints[slot].lib_name) - 1] = '\0';
  g_hookpoints[slot].index = -1;
  g_hookpoints[slot].active = true;
  ...
  DI_LOGI("hookpoint: set on %s init_array[*]", lib_name);
  return 0;
}
