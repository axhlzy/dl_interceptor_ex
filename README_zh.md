...
- **GumTrace 支持** — 使用 `libGumTrace.so` 对特定的 init 函数进行深层执行流追踪
- **Init array Hooks** — 替换、跳过或包装 (Wrap) 特定的 init 函数，并测量其执行耗时 (ms)
...
# 自动检测设备架构，按需编译、推送到手机、自动清空 logcat 并注入到应用中
...
选填:
  ...
  -t <lib:N>      使用 GumTrace 深度追踪指定 init_array 函数执行逻辑 (基于 1，可指定多个)
                  格式: libname.so:N | libname.so:* | libname.so
  -a <activity>   用于调起应用的 Activity 名 (默认: 自动检测)
  -w <timeout>    等待进程启动的超时时间 ms (默认: 10000)
...
# 使用 GumTrace 深度追踪 libfoo.so 的第1个 init_array 函数
di_injector -p com.example.app -t libfoo.so:1
# 日志存储路径: /data/local/tmp/trace/<包名>/libfoo_1_0x<地址>.log
...
6. 远程调用 `dl_interceptor_set_package_name()` 以及各类配置函数 (`set_breakpoint`, `set_tracepoint` 等)
7. `ptrace(DETACH)` — 目标进程恢复运行并使钩子正常生效
8. 如开启 Trace，在目标进程中加载 `libGumTrace.so` 并向 `/data/local/tmp/trace/` 写入日志
...
// 使用 GumTrace 追踪特定的 init_array 执行
...
- **[GumTrace](https://github.com/lidongyooo/GumTrace)**: 基于 Frida Gum 开发的强大执行流追踪工具，已被集成到本项目用于深层函数逻辑追踪。
